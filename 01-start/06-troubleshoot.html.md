...
To do this, add the following to your [DocPad configuration file](https://docpad.org/docs/config):
...
Add the following to your [DocPad configuration file](https://docpad.org/docs/config):
...
This occurs when you run `docpad run` inside a directory that already has existing files, but doesn't have a structure that resembles a DocPad project. We can't directly ask you if you would like to use an existing [skeleton](https://docpad.org/docs/skeletons) for the basis of your new website, as pulling in a skeleton inside a non-empty directory may overwrite your existing files. If would like to still use a skeleton for the basis of your new website, you will have to run DocPad inside a new empty directory. If you would like to start your website from scratch (not use an existing skeleton) then you can follow the [Getting Started](https://docpad.org/docs/start) guide. Hope that helps :) [If you need more help then check out our Support Channels](https://discuss.bevry.me/t/getting-support-guide/63/1).
...
- [Set `standalone: true` to the meta data of documents that you regenerate often](https://docpad.org/docs/meta-data#standalone)
...
[Check out the Upgrade Guides here](https://docpad.org/docs/upgrade)
